...
class HybridRetriever:
    ...
    async def _get_all_documents(
        ...
        try:
            ...
            # 使用ChromaDB的get方法获取所有文档
            # ChromaDB 1.5.x 支持直接获取所有文档
            try:
                results = self.vector_store.collection.get(
                    include=["documents", "metadatas"],
                    limit=min(count, 5000)  # 限制最大数量
                )
                ...
                logger.warning(f"Direct collection access failed: {e}, using fallback")
                # 回退方案：使用空查询获取所有文档
                results = await self.vector_store.search(
                    query="",
                    top_k=min(count, 5000)
                )
                return results
